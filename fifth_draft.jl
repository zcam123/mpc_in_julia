...
function control_loop(steps, Zs, initial_state, f, p, lcons, ucons, lb, ub, prediction_horizon, control_horizon) #should add other funcs that appear here as args
    ...
    for _ in 1:steps
        ...
        access = solution.u
        start_state = [0 for _ in (prediction_horizon*4+control_horizon)];
        start_state[1:4] = access[5:8] #making first state for next loop equal to result of applying firs optimized input
        #get rest of states
